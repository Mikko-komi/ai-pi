> 本文为 [environment-variables.md](environment-variables.md) 的中文译本。

# 环境变量

Pi 以三种方式使用环境变量：

- 如 `PI_OFFLINE` 这类变量配置 Pi 进程。
- Pi 设置进程标记，让子进程能识别启动它们的 agent 是 Pi。
- LLM 可调用的 shell 工具所运行的命令会收到描述当前会话的 `PI_*` 变量。

提供方 API key 变量另见[提供方](providers.zh.md#环境变量或鉴权文件)。

## 进程标记

CLI 和 RPC 入口会设置两个进程标记：

- `AI_AGENT=pi` 是通用标记，让工具能识别启动该进程的 agent 是 Pi。
- `PI_CODING_AGENT=true` 是 Pi 专用标记，让子进程检测自己运行在 Pi 内部。

子进程会继承这两个标记。它们与具体会话无关；通过 SDK 嵌入 Pi 时不会自动设置。
...
这些值在每条命令启动时解析。因此切换模型或更改推理级别会影响下一条 shell 命令，无需重启 Pi。`PI_PROVIDER` 和 `PI_MODEL` 标识的是选中的 Pi 模型，而不是 router 内部可能选择的另一个上游模型。

当被问到当前运行的是哪个模型或提供方时，请检查这些变量，而不是从系统提示推断：
...
用 `createBashTool()` 或 `createPowerShellTool()` 创建、并注册到 Pi 的工具默认会暴露会话环境。注入发生在 `spawnHook` 之前，因此 hook 会在 `ctx.env` 中收到这些变量：
...
| `PI_TELEMETRY` | 覆盖安装/更新遥测和提供方归因头：`1`/`true`/`yes` 或 `0`/`false`/`no` |
| `PI_CACHE_RETENTION` | 设为 `long` 可在支持时延长提供方提示缓存 |
...
| `PI_TUI_ESC_TIMEOUT` | 单独收到 ESC 后等待多久才将其视为 Escape，单位毫秒；SSH 上默认为 `100`，否则为 `10`。若 Alt 键输入被误读为 Escape，请增大该值 |
| `VISUAL`、`EDITOR` | 未设置 `externalEditor` 时的外部编辑器回退 |
| `HTTP_PROXY`、`HTTPS_PROXY` | 代理出站 HTTP 请求 |

`ANTHROPIC_API_KEY`、`OPENAI_API_KEY` 等提供方凭证以及云提供方配置列在[提供方](providers.zh.md#环境变量或鉴权文件)。

`PI_SERVER_DIR` 和 `PI_SERVER_ID` 仅适用于仅源码提供的[实验性远程 harness](development.zh.md#实验性远程-harness)，不适用于已分发的构建。
