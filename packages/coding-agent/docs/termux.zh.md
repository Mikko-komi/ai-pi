...
在 Termux 中运行时，剪贴板操作使用 `termux-clipboard-set` 和 `termux-clipboard-get`。这些命令需要安装 Termux:API 应用。
...
- **存储访问**：要访问 `/storage/emulated/0`（Downloads 等）中的文件，请先运行一次 `termux-setup-storage` 以授予权限
