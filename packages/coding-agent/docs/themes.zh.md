> 本文为 [themes.md](themes.md) 的中文译本。

> 可以让 pi 创建主题。请它按你的环境构建一个。
...
- 软件包：`themes/` 目录，或 `package.json` 中的 `pi.themes` 条目
- 设置：`themes` 数组，可为文件或目录
- CLI：`--theme <path>`（可重复）
...
在不改已保存设置的情况下，为一轮交互指定主题：

```bash
pi --use-theme light
```

若要跟随终端外观，使用 `lightTheme/darkTheme` 语法：

```bash
pi --use-theme light/dark
```

CLI 值是该轮运行的初始主题。之后在 `/settings` 中选择其他主题会立即生效，并按常规定保存。
...
2. 用全部必需颜色定义主题（见[颜色 token](#颜色-token)）：
...
**热重载：** 编辑当前活动的自定义主题文件时，pi 会自动重新加载，以便立即看到视觉效果。
...
- `colors` 必须定义全部 53 个必需 token。`thinkingMax` 和两个搜索高亮 token 是可选的，使用下面列出的回退。

`$schema` 字段可启用编辑器自动补全和校验。

## 颜色 token

每个主题必须定义全部 53 个必需颜色 token。可选 token 用于兼容已有主题：`thinkingMax` 回退到 `thinkingXhigh`，`searchMatchBg` 回退到 `selectedBg`，`searchMatchText` 回退到 `text`。其他搜索匹配使用 `searchMatchBg` 上的 `searchMatchText` 并加下划线；当前匹配反转该前景/背景对并使用粗体。
...
### Thinking 级别边框（6 个必需，1 个可选）

表示 thinking 级别的编辑器边框颜色（从弱到强的视觉层次）：
...
`export` 段控制 `/export` HTML 输出的颜色。若省略，颜色从 `userMessageBg` 派生。
...
Pi 使用 24 位 RGB 颜色。大多数现代终端支持（iTerm2、Kitty、WezTerm、Windows Terminal、VS Code）。对仅支持 256 色的旧终端，pi 会回退到最接近的近似值。
...
**色彩和谐：** 从一套基础调色板开始（Nord、Gruvbox、Tokyo Night），在 `vars` 中定义，并保持一致引用。

**测试：** 用不同类型的消息、工具状态、markdown 内容和长换行文本检查主题。
